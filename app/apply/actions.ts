...
export async function submitApplication(data: ApplicationData) {
  try {
    ...
    try {
      if (process.env.RESEND_API_KEY) {
        const { data: adminEmailData, error: adminEmailError } =
          await resend.emails.send({
            ...
            <ul>
              <li><strong>Business Name:</strong> ${data.businessName}</li>
              <li><strong>Business Address:</strong> ${data.businessAddress}, ${
              data.businessCity
            }, ${data.businessState} ${data.businessZip}</li>
              <li><strong>Years in Business:</strong> ${
            ...
            <ul>
              ...
              <li><strong>SMS Consent:</strong> ${
                data.smsConsent
                  ? "Yes - Consented to receive SMS messages"
                  : "No - Did not consent to SMS messages"
              }</li>
            </ul>
            <h2>Financial Information:</h2>
            <ul>
              <li><strong>Monthly Revenue:</strong> ${data.monthlyRevenue}</li>
              <li><strong>Requested Amount:</strong> ${
      ...
      console.error(
        "Exception sending admin notification email:",
        adminEmailError
      );
    }
